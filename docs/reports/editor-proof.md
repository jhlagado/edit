# Edit proof history and standalone checkpoint

Original checkpoint: 2026-08-26

Standalone ATOM qualification: 2026-09-05
...
[`specification.md`](../specification.md) and the retained buffer
measurement in
[`buffer-measurement.md`](buffer-measurement.md).
...
implementation is `dddcbd68`. That historical checkpoint used the then-current
register-contract oracle. The standalone repository now assembles only with
ATOM and reproduces the same 3,003-byte artifact exactly.
...
`npm test` assembles the production sources with the pinned ATOM revision and
executes them through the standalone proof. Debug80 Runtime supplies only the
development-time Z80 and terminal model. BDOS calls cross the real register
interface into a deterministic proof adapter. The adapter implements only the CP/M operations
...
At the original Debug80 checkpoint, these integration gates passed:

- the isolated proof covered the production editor paths and exact accounts
  above;
...
- `npm run check` passes build, typecheck, lint, formatting, and the full test
  suite across the then-current assembler, Glimmer, the runtime, both headless integrations, Debug80,
...
These are retained historical consumer results. The current standalone release
gate is `npm run check`: it performs the deterministic ATOM build and the
isolated Z80/BDOS/terminal proof. Triptych, Debug80, and other consumers own
their additional disk-image and UI integration tests.
