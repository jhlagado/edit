...
import {
  ...
  renderAtomArtifacts,
  translateAzmSourceToAtom as translateLegacySourceToAtom,
} from "atom-z80";
...
export function projectBdosBiosEqu(source, originText, expectedBytes) {
  const finalCursor = parseFixedNumber(originText) + expectedBytes.length;
...
export function projectConvertedAtomSource(candidate, source, expectedBytes) {
  let projected = source;
  if (candidate.name === "bdos") {
    projected = projectBdosBiosEqu(projected, candidate.origin, expectedBytes);
  }
  ...
  return source
    ...
    .map((line) =>
      rewriteCodeOutsideText(line, (code) =>
        code.replace(
          /^(\s*)\.binto\s+(.+?)\s*$/i,
          (_match, indent, end) => `${indent};@OUTPUT-END ${end.trim()}`,
        ),
      ),
    )
    .join("\n");
...
export function projectLegacyConditionDirectives(source) {
  return source
    .split(/\r\n|\n|\r/)
    .map((line) =>
      rewriteCodeOutsideText(line, (code) =>
...
function projectLegacyFixtureDirective(line) {
...
export function projectLegacyFixtureSyntaxToAtom(source) {
  ...
  const lines = source.split(/\r\n|\n|\r/).map((line) => {
    ...
    return projectLegacyFixtureDirective(line);
  });
...
export function requireByteIdentity(label, expected, actual) {
  if (
    ...
    fail(`${label}: Atom output differs from expected output`);
  }
}
...
export async function assembleConvertedWithAtom(options) {
  ...
  const atomBytes = await assembleAtomSource(
    ...
    { start, length: options.expectedBytes.length },
  );
  requireByteIdentity(options.candidate.name, options.expectedBytes, atomBytes);
  return atomBytes;
}
...
export async function assembleProjectOwnedWithAtom({
  ...
  const projected =
    typeof candidate === "object"
      ...
      : source;
  const atomSource = translateLegacySourceToAtom(projected, { sourceName: name });
  ...
  requireByteIdentity(name, expectedBytes, atomBytes);
  return atomBytes;
}
...
export async function assembleProjectOwnedAtomArtifacts({
  ...
  const atomSource = translateLegacySourceToAtom(projected.source, {
    sourceName: name,
  });
  await writeFile(join(temporaryDirectory, name), atomSource, "utf8");
  const result = await assembleAtomProject({
...
export async function assembleLegacySourceWithAtomArtifacts({
  temporaryDirectory,
  sourceDirectory,
  name,
  source,
  ...
  const fixtureSource = withMissingExternEquates(
    projectLegacyFixtureSyntaxToAtom(projectOutputRangeDirective(expanded)),
    externalSymbols,
  );
